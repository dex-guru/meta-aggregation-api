...
from web3 import HTTPProvider, AsyncHTTPProvider
...
def _on_session_evicted_from_cache(cache_key, session: requests.Session) -> None:
    ...
    _logger.info(f"Closed http session: %({LogArgs.web3_url})s", log_args, extra=log_args)


def _on_async_session_evicted_from_cache(cache_key, session: ClientSession) -> None:
    session.close()
    log_args = {LogArgs.web3_url: cache_key, LogArgs.cache_size: len(_async_session_cache)}
    _logger.info(f"Closed async http session: %({LogArgs.web3_url})s", log_args, extra=log_args)
...
def _get_session(endpoint_uri: URI) -> requests.Session:
    ...
    if cache_key not in _session_cache:
        ...
        http_adapter = HTTPAdapter(max_retries=Retry(connect=5, read=3), pool_connections=32, pool_maxsize=32)
        session.mount("https://", http_adapter)
        session.mount("http://", http_adapter)
        with _session_cache_lock:
            _session_cache[cache_key] = session
            log_args = {LogArgs.web3_url: cache_key, LogArgs.cache_size: len(_session_cache)}
            _logger.info(f"Created http session: %({LogArgs.web3_url})s", log_args, extra=log_args)
...
async def _get_async_session(endpoint_uri: URI) -> ClientSession:
    ...
    if cache_key not in _async_session_cache:
        ...
        with _async_session_cache_lock:
            ...
            log_args = {LogArgs.web3_url: cache_key, LogArgs.cache_size: len(_async_session_cache)}
            _logger.info(f"Created async http session: %({LogArgs.web3_url})s", log_args, extra=log_args)

    return _async_session_cache[cache_key]


def _make_post_request(endpoint_uri: URI, data: bytes, *args: Any, **kwargs: Any) -> bytes:
...
class CustomHTTPProvider(HTTPProvider):
    def make_request(self, method: RPCEndpoint, params: Any) -> RPCResponse:
        ...
        raw_response = _make_post_request(
            self.endpoint_uri,
            request_data,
            **self.get_request_kwargs()  # type: ignore # see to_dict decorator on the method
        )
