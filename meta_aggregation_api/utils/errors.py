...
from meta_aggregation_api.utils.logger import LogArgs
...
class BaseAggregationProviderError(Exception):
    ...
    def __repr__(self):
        return (
            f'{self.__class__.__name__}({self.provider}, {self.message}, {self.kwargs})'
        )
...
responses = {
    UserMistakes.code: {
        ...
        % (
            UserBalanceError.msg_to_log,
            TokensError.msg_to_log,
            EstimationError.msg_to_log,
            AllowanceError.msg_to_log,
        )
    },
    ProviderMistakes.code: {
        'description': 'One of the following errors:<br><br>%s<br>%s<br>%s<br>%s'
        % (
            ProviderTimeoutError.msg_to_log,
            InsufficientLiquidityError.msg_to_log,
            PriceError.msg_to_log,
            AggregationProviderError.msg_to_log,
        )
    },
    OurMistakes.code: {
        'description': 'One of the following errors:<br><br>%s<br>%s<br>%s<br>%s'
        % (
            ProviderNotFound.msg_to_log,
            SpenderAddressNotFound.msg_to_log,
            ParseResponseError.msg_to_log,
            ValidationFailedError.msg_to_log,
        )
    },
}
