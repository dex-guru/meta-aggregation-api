...
from meta_aggregation_api.services.meta_aggregation_service import get_token_allowance, get_approve_cost, get_approve_costs_per_provider, \
...
async def test_get_approve_costs_per_provider(sell_amount: int, approve_called: bool):
    ...
    erc20_contract = Mock()
    allowance_patcher = patch('services.meta_aggregation_service.get_token_allowance')
    approve_patcher = patch('services.meta_aggregation_service.get_approve_cost')
...
@pytest.mark.asyncio()
@patch('providers.zerox_v1.ZeroXProviderV1.get_swap_price', new_callable=AsyncMock)
@patch('providers.one_inch_v5.OneInchProviderV5.get_swap_price', new_callable=AsyncMock)
async def test_get_swap_meta_price_no_price(
    ...
    approve_patcher = patch('services.meta_aggregation_service.get_approve_costs_per_provider')
    ...
    with pytest.raises(ValueError, match='No prices found'):
        await get_swap_meta_price(
            test_str, test_str_2, test_int, test_int, test_int, test_int, test_str, test_str, None)


@pytest.mark.asyncio()
@patch('services.meta_aggregation_service.DexGuru.get_token_inventory_by_address', new_callable=AsyncMock)
@pytest.mark.parametrize(
    'token_address, call_count',
    (
            ('test_token', 1),
            (config.NATIVE_TOKEN_ADDRESS, 0),
            (chains.eth.native_token.address, 0)
    )
)
